// da11.cpp : Defines the entry point for the console application.
//
...
typedef struct Edge 
{
    ...
    float cost;
    char color[8];
    int thickness;// Assuming the color is stored as a string with a maximum length of 7 characters (plus null-terminator)
} Edge;
...
void start()
{
	...
	void __cdecl KeyboardFunc(unsigned char key, int x, int y);
	glFlush();
}
